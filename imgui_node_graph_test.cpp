...
static void ShowExampleAppCustomNodeGraph(bool* opened)
{
    ...
    // Draw a list of nodes on the left side 左側にノードのリストを描画
    ...
    // Create our child canvas 子キャンバスを作成
    ...
    for (int node_idx = 0; node_idx < nodes.Size; node_idx++)
    {
        ...
        ImGui::BeginGroup(); // Lock horizontal position 水平位置をロック
        ImGui::Text("%s", node->Name);
        ImGui::SliderFloat("##value", &node->Value, 0.0f, 1.0f, "Alpha %.2f");
        ImGui::ColorEdit3("##color", &node->Color.x);
        ImGui::EndGroup();

        // Save the size of what we have emitted and whether any of the widgets are being used 出力したもののサイズと、ウィジェットが使用されているかどうかを保存
        bool node_widgets_active = (!old_any_active && ImGui::IsAnyItemActive());
        node->Size = ImGui::GetItemRectSize() + NODE_WINDOW_PADDING + NODE_WINDOW_PADDING;
        ImVec2 node_rect_max = node_rect_min + node->Size;

        // Display node box ノードボックスを表示
        draw_list->ChannelsSetCurrent(0); // Background
    ...
    // Draw context menu コンテキストメニューを描画
